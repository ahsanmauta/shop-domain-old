      ...
      <div class="card-body">
		...
		@endphp
		  ...
		  <li class="d-flex mb-6">
            <div class="me-4">
			  <form action="{{route('users.winsave')}}" method="POST" id="formbid{{ $i }}" >  
			  @csrf
			  <input type="hidden" id="idbid" name="idbid" value="{{ $bid->id }}" />
			  <div class="dropdown z-2">
				  <button type="button" class="btn btn-icon btn-text-secondary rounded-pill dropdown-toggle hide-arrow p-0" data-bs-toggle="dropdown" aria-expanded="false"><i class="ti ti-dots-vertical ti-md text-muted"></i></button>
				  <ul class="dropdown-menu dropdown-menu-end">
					<li><a class="dropdown-item" href="javascript:void(0);" onclick="document.getElementById('formbid{{ $i }}').submit()" >Winner</a></li>
				  </ul>
				</div>
			  </form>
            </div>
            <div class="d-flex w-100 flex-wrap align-items-center justify-content-between gap-2">
              <div class="me-2">
                <h6 class="mb-0">{{ $bid->firstname.' '.$bid->lastname }}</h6>
                <small class="text-body d-block">Email: {{ $bid->email }}</small>
				<small class="text-body d-block">Status: 
				@php
					if($bid->bidstatus==1){
						echo "open";
					}
              ...
              <div class="user-progress d-flex align-items-center gap-1">
                <p class="mb-0">${{ number_format($bid->bidprice,2,".",",") }}</p>
              </div>
            </div>
          </li>
		  @endforeach
